...
--insert (dynamic partitions) 
	-->> cat_dept_id must be the last column in the select statement
insert into categories_partitionedby_deptid partition(cat_dept_id) 
	select cat_id, cat_name, cat_dept_id from categories;

--insert (not dynamic partitions) 
	-->> notice that we did not select 'cat_dept_id' column
insert into categories_partitionedby_deptid partition(cat_dept_id=11) 
	select cat_id, cat_namefrom categories;
